...
def play_game():
    ...
    while True:
        ...
        # Check if there's any match on the board
        matched = player.board.clearMatches()
        if matched:
            print("\nMatches found and cleared!")
            player.board.applyGravity()  # Apply gravity after clearing matches
            print("New Board after gravity and fill:")
            print(player.board.getBoardDisplay())
        ...
        if current_tile_shape.tiles[0].position[0] == player.board.height - 1:
            spawnCurrentTileShape()
        # Checks if landed on another tile
